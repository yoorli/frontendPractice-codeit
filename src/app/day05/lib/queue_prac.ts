...
function queue(arr: number[], location: number): number {
  // 큐에 우선순위와 원래 인덱스를 함께 저장
  const q = arr.map((priority, index) => ({ priority, index }));
  let printed = 0;

  while (q.length) {
    const cur = q.shift()!; // 맨 앞 문서
    // 큐 안에 cur보다 우선순위가 높은 문서가 있으면 뒤로 보냄
    if (q.some((x) => x.priority > cur.priority)) {
      q.push(cur);
      continue;
    }
    // 그렇지 않으면 출력
    printed += 1;
    if (cur.index === location) return printed; // 목표 문서 출력 시 순서 반환
  }
  return printed;
}
